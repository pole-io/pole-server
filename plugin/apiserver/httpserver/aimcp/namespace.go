...
func (h *HTTPServer) addToolQueryNamespaces(mcpSvr *server.MCPServer) {
	mcpSvr.AddTool(
		...
		h.handleQueryNamespaces)
}

func (h *HTTPServer) handleQueryNamespaces(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func (h *HTTPServer) addToolCreateNamespaces(mcpSvr *server.MCPServer) {
	mcpSvr.AddTool(
		mcp.NewTool("create_namespaces",
			mcp.WithDescription("创建命名空间"),
			mcp.WithArray("namespaces",
				mcp.Description("命名空间数组"),
				mcp.Items(httpcommon.MarshalPBJsonToMap(&apimodel.Namespace{})),
			),
		),
		h.handleCreateNamespaces)
}

func (h *HTTPServer) handleCreateNamespaces(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func (h *HTTPServer) addToolDeleteNamespaces(mcpSvr *server.MCPServer) {
	mcpSvr.AddTool(
		...
		h.handleDeleteNamespaces)
}

func (h *HTTPServer) handleDeleteNamespaces(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func (h *HTTPServer) addToolUpdateNamespaces(mcpSvr *server.MCPServer) {
	mcpSvr.AddTool(
		mcp.NewTool("update_namespaces",
			mcp.WithDescription("更新命名空间"),
			mcp.WithArray("namespaces",
				mcp.Description("命名空间数组"),
				mcp.Items(httpcommon.MarshalPBJsonToMap(&apimodel.Namespace{})),
			),
		),
		h.handleUpdatwNamespaces)
}

func (h *HTTPServer) handleUpdatwNamespaces(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
