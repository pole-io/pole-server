...
import (
	...
	limitconfig "github.com/pole-io/pole-server/pkg/limiter/internal/config"
	"github.com/pole-io/pole-server/pkg/limiter/internal/statistics"
)

// APIServerConfig 描述 Limiter 的一个协议监听器。
type APIServerConfig = apiserver.Config

// LimitConfig 描述 Limiter 核心运行参数。
type LimitConfig = limitconfig.Config

// StatisticsConfig 描述 Limiter 统计实现配置。
type StatisticsConfig = statistics.Config

// Config 描述 Limiter 运行模块配置。
type Config struct {
	Registry   RegistryConfig    `yaml:"registry"`
	APIServers []APIServerConfig `yaml:"api-servers"`
	Limit      LimitConfig       `yaml:"limit"`
	Plugin     StatisticsConfig  `yaml:"plugin"`
}
...
// ParseListenOption 返回监听地址和端口，供进程启动前执行端口冲突检查。
func ParseListenOption(option map[string]interface{}) (string, uint32, error) {
	return apiserver.ParseListenOption(option)
}
