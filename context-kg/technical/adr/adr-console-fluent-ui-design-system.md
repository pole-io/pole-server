...
- 单一文本条件的列表默认只展示一个主搜索框；多条件列表使用应用级 `QueryComposer`，由主搜索、自动补全、已选条件 Tag 和高级筛选浮层组成，不再把全部输入与选择器平铺在工具栏。
- 主搜索框只承接页面已有的主查询语义，不得在后端不支持时虚构跨字段全文搜索；结构化文本、单选、多选和三态布尔条件进入高级筛选。
- 高级筛选使用草稿状态，取消、Esc 或点击外部不写回半成品；确认后以可删除 Tag 展示已选条件。远程数据页显式点击查询，本地数据页即时过滤，同一页面不得混用两种提交语义。
- 日期时间范围始终作为主查询行的独立控件，不进入高级筛选或条件 Tag；查询与重置继续沿用页面原有 API 参数和第一页语义。
- 桌面端主搜索、独立时间、筛选和操作可以同排并安全换行；720px 以下高级筛选切换为单列、内部滚动且底部操作保持可达。
...
- 命名空间、服务、配置分组、治理工作台、MCP、A2A、系统配置和观测多条件页已接入统一 `QueryComposer`；单字段认证、治理子表和详情内局部搜索继续保持轻量主搜索，不制造空高级筛选。
