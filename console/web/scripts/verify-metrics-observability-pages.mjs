...
const checks = [
  ...
  },
  {
    ...
    expectations: [
      ...
      ['renders grafana variable names', /\$category[\s\S]*\$api[\s\S]*\$component/],
      ['renders variable labels with grafana names', /<VariableItem name="\$category" label="类别"[\s\S]*<VariableItem name="\$api" label="接口"[\s\S]*<VariableItem name="\$component" label="组件"/],
      ['uses lightweight variable reset action', /className=\{style\.variableReset\}/],
      ['contains category filter', /placeholder="类别"/],
      ['contains api filter', /placeholder="接口"/],
  ...
  },
  {
    ...
    expectations: [
      ...
      ['contains service info filter', /placeholder="服务信息"/],
      ['contains governance filter', /placeholder="治理能力"/],
