...
assert.match(styleSource, /\.laneRatioControl\s+:global\(\.fluent-input-adornment\)\s*\{[\s\S]*?min-width:\s*104px;/);
assert.match(styleSource, /\.laneRatioControl\s+:global\(\.fui-Input\)[\s\S]*?min-width:\s*0;/);
assert.match(styleSource, /\.formPane\s*\{[\s\S]*?overflow:\s*auto;/);
assert.match(styleSource, /:global\(\.fluent-tabs\)\s*\{[\s\S]*?min-height:\s*100%;/);
assert.match(styleSource, /:global\(\.fluent-tab-content\)\s*\{[\s\S]*?overflow:\s*visible;/);
assert.match(styleSource, /\.segmentButton\s*\{[\s\S]*?height:\s*32px;/);
assert.match(styleSource, /\.laneCardBody\s+:global\(\.fui-Input\),/);
assert.match(styleSource, /\.laneRatioControl\s+:global\(\.fluent-number-input\)\s*\{[\s\S]*?width:\s*100%;/);
