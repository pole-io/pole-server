...
const (
	DefaultAgentProposalTTL     = 30 * time.Minute
	DefaultAgentUpstreamTimeout = 10 * time.Second
)
...
func NewSystemSettingsProvider(cfg *Config) (systemconfig.EffectiveProvider, error) {
	...
	registrations := []systemconfig.Registration{
		...
		consoleSetting("console.web.web_path", "runtime", "静态资源目录", "webServer.webPath", systemconfig.ValueTypeString, systemconfig.RestartRequired, systemconfig.SensitivityInternal, func() any { return cfg.WebServer.WebPath }),
...
func consoleSystemSettingPolicies() map[string]systemconfig.EditPolicy {
	...
	return map[string]systemconfig.EditPolicy{
		...
		"console.web.web_path":    locked("静态资源目录属于镜像工件", "由 Console 镜像构建和挂载方式确定。"),
