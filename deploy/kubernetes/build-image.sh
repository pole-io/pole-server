...
POLE_CONSOLE_NPM_CI=1 "${root_dir}/scripts/build-console-assets.sh" release

printf '[pole-k8s] build linux/%s binary\n' "${goarch}"
(
  cd "${root_dir}"
  CGO_ENABLED=0 GOOS=linux GOARCH="${goarch}" go build -tags "nomsgpack consoleassets" -o pole-server \
